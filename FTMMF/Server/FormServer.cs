...
namespace Server
{
    ...
    {
        MMFServer m_Server = new MMFServer();
        ...
        {
            InitializeComponent();
            m_Server.LoopInterval = 100;
            m_Server.GetDiValue+=new MMFServer.DICallBack(m_Server_GetDiValue);
            m_Server.GetMsg+=new MMFServer.MsgCallBack(m_Server_GetMsg);
            m_Server.GetDOValue += new MMFServer.DOCallBack(m_Server_GetDOValue);
            DiList.Add(DI_0);
            DiList.Add(DI_1);
            DiList.Add(DI_2);
            DiList.Add(DI_3);
        ...
        private void DO_0_CheckedChanged(object sender, EventArgs e)
        {
            ...
            m_Server.DO = Common.SetIntSomeBit(m_Server.DO, i_Num, tmp.Checked);
        }
        private  delegate void SetDo(int Do);
        private void m_Server_GetDOValue(int Do)
        {
            BeginInvoke(new SetDo(SetDOUI),Do);
        }
        private delegate void SetDi(int Di);
        private void m_Server_GetDiValue(int Di)
        {
            BeginInvoke(new SetDi(SetDIUI), Di);
        }
        private delegate void SetMsg(string Msg);
        private void m_Server_GetMsg(string Msg)
        {
        ...
        {
            m_Server.SendMessage(textBox1.Text);
        }

        private void button2_Click(object sender, EventArgs e)
        {
            m_Server.Start();
            SetMsgUI("Server Start");
        }
            
        private void button3_Click(object sender, EventArgs e)
        {
            m_Server.Stop();
            SetMsgUI("Server Stop");
        }
